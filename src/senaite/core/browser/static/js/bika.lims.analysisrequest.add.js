...
// === Auto SampleType desde múltiples Perfiles ===
AnalysisRequestAdd.prototype.set_sample_type_from_profiles = function(arnum){
  try{
    var $profiles = $(`tr[fieldname=Profiles] textarea#Profiles-${arnum}`);
    var profCtl = this.get_widget_controller($profiles);
    if (!profCtl) return;

    var selectedUIDs = profCtl.get_values() || [];
    if (selectedUIDs.length === 0) return;

    var record = this.records_snapshot[arnum] || {};
    var pm = record.profiles_metadata || {};

    // Extrae "Profile Keyword" de cada perfil seleccionado (múltiples variantes)
    var keywords = [];
    selectedUIDs.forEach(function(uid){
      var meta = pm[uid] || {};
      var k =
        (meta["Profile Keyword"] || "").trim() ||
        (meta.getProfileKeyword || "").trim() ||
        (meta.ProfileKeyword || "").trim() ||
        (meta.getProfileKey || "").trim() ||
        (meta.ProfileKey || "").trim() ||
        (meta.profile_keyword || "").trim() ||
        (meta.profilekeyword || "").trim();
    ...
    var allSame = keywords.every(function(k){ return k === first; });

    var $sampleType = $(`#SampleType-${arnum}`);
    var currentVal = this.get_reference_field_value($sampleType);
    var stCtl = this.get_widget_controller($sampleType);
    if (!stCtl) return;

    if (allSame) {
      // Busca un SampleType cuyo "keyword" coincida con first y lo establece si el campo está vacío
      var that = this;
      stCtl.search(first).then(function(data){
        if (!data || data.count !== 1) return; // evita ambigüedades
        if (!currentVal) {
          var item = data.items[0];
          that.set_reference_field($sampleType, item.uid);
        }
      }).catch(function(e){ console.warn(e); });
    } else {
      // Conflicto: diferentes Profile Keyword → no autocompletar
      try {
        site.add_notification(
          _t("Perfiles con 'Profile Keyword' distintos"),
          _t("No se cambia 'Tipo de muestra'. Elige uno manualmente.")
        );
      } catch(e) {}
    }
...
// Llama a la función al seleccionar/deseleccionar perfiles
(function(proto){
  var origSelected = proto.on_analysis_profile_selected;
  proto.on_analysis_profile_selected = function(event){
    if (origSelected) origSelected.call(this, event);
    var $el = $(event.currentTarget);
    var arnum = $el.closest("[arnum]").attr("arnum");
    this.set_sample_type_from_profiles(arnum);
  };

  var origRemoved = proto.on_analysis_profile_removed;
  proto.on_analysis_profile_removed = function(event){
    if (origRemoved) origRemoved.call(this, event);
    var $el = $(event.currentTarget);
    var arnum = $el.closest("[arnum]").attr("arnum");
    this.set_sample_type_from_profiles(arnum);
  };
})(window.AnalysisRequestAdd.prototype);
